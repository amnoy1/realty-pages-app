    ...
    <div className="max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-xl space-y-6">
        <div>
          ...
          <input type="text" id="fullName" name="fullName" className="w-full px-4 py-2 bg-slate-800 text-white border border-slate-600 rounded-lg shadow-sm focus:ring-slate-500 focus:border-slate-500 transition duration-150 placeholder-slate-400" placeholder="ישראל ישראלי" value={fullName} onChange={(e) => setFullName(e.target.value)} required />
        </div>
        <div>
          <label htmlFor="phone" className="block text-sm font-medium text-gray-700 mb-1">טלפון</label>
          <input type="tel" id="phone" name="phone" className="w-full px-4 py-2 bg-slate-800 text-white border border-slate-600 rounded-lg shadow-sm focus:ring-slate-500 focus:border-slate-500 transition duration-150 placeholder-slate-400" placeholder="050-123-4567" value={phone} onChange={(e) => setPhone(e.target.value)} required />
        </div>
        <button type="submit" className="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-lg text-lg font-medium text-white bg-slate-700 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 transition-all duration-300 transform hover:scale-105">שלח וקבע סיור</button>
      </form>
    </div>
  );
};
