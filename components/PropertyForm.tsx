      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <InputField name="agentWhatsApp" label="מספר ווצאפ של הסוכן" placeholder="בפורמט בינלאומי, לדוגמה: 972501234567" value={formData.agentWhatsApp} onChange={handleChange} required />
           ...
           <div>
            ...
            <div className="mt-1 flex items-center">
                {logo ? (
                    <img src={logo} alt="logo preview" className="h-16 w-16 object-contain rounded-md bg-gray-100 p-1 mr-4" />
                ) : (
                    <div className="h-16 w-16 bg-gray-100 rounded-md flex items-center justify-center mr-4">
                        <LogoIcon />
                    </div>
                )}
                <label htmlFor="logo-upload" className="relative cursor-pointer bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500">
                    <span>העלה לוגו</span>
            ...
            {images.length > 0 && (
                <div className="mt-4 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                    {images.map((img, index) => (
                        ...
                        >
                            <img src={img} alt={`preview ${index}`} className="h-24 w-full object-cover rounded-lg shadow-md pointer-events-none" />
                             <button
                                type="button"
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-lg text-lg font-medium text-white bg-slate-700 hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 disabled:bg-slate-400 transition-all duration-300 transform hover:scale-105"
          >
            {isLoading ? (
              <>
                <Spinner />
