...
// --- Feature Icons with Explicit Sizes ---
...
const FeatureItem: React.FC<{ icon: React.ReactNode; label: string; value?: string }> = ({ icon, label, value }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
      <div className="mb-3 p-2 bg-slate-50 rounded-full text-brand-accent">{icon}</div>
...
const KeyFeatureItem: React.FC<{ icon: React.ReactNode; label: string; value?: string }> = ({ icon, label, value }) => {
    if (!value) return null;
    return (
        <div className="flex items-center gap-3 text-white bg-black/30 backdrop-blur-md p-3 rounded-xl border border-white/10">
...
const FeaturesSection: React.FC<{ features: PropertyFeatures }> = ({ features }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
        <h3 className="text-xl font-bold text-slate-800 mb-6 text-center">מאפייני הנכס</h3>
...
export const LandingPage: React.FC<LandingPageProps> = ({ details, isPreview = false, onReset, onSave, isSaving }) => {
  ...
  return (
    <div className="bg-brand-light min-h-screen text-slate-800 font-sans">
        {!isPreview && (
            <a href={`https://wa.me/${details.agentWhatsApp}`} target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 bg-[#25D366] text-white p-4 rounded-full shadow-2xl z-50 hover:scale-110 transition-transform" aria-label="צור קשר ב-WhatsApp">
                <WhatsAppIcon/>
            </a>
        )}
      <header className="relative h-[85vh] min-h-[600px] text-white overflow-hidden">
        ...
        {details.logo && (
          <div className="absolute top-6 right-6 bg-white/90 backdrop-blur p-3 rounded-xl shadow-lg">
             <Image 
                src={details.logo} 
                alt="לוגו המשרד" 
                width={140}
        ...
        <div className="absolute bottom-0 right-0 w-full p-6 md:p-16 pb-20">
            <div className="container mx-auto max-w-7xl">
                <div className="max-w-4xl animate-slide-up">
                    ...
                    <p className="text-2xl md:text-3xl font-light text-slate-200 mb-8 flex items-center gap-2">
                        <span className="inline-block w-1 h-8 bg-brand-accent mr-2"></span>
                        {details.address}
                    </p>
                    ...
                    <button
                        ...
                        className="py-4 px-10 rounded-full shadow-xl shadow-orange-500/20 text-xl font-bold text-white bg-brand-accent hover:bg-brand-accentHover transition-all duration-300 transform hover:-translate-y-1"
                    >
                        תיאום סיור בנכס
                    </button>
                </div>
        ...
        <div className="absolute top-6 left-6 flex flex-col items-start gap-3 z-50">
           {isPreview && onReset && (
                <button 
                    onClick={onReset} 
                    className="bg-black/60 text-white py-2 px-5 rounded-full hover:bg-black/80 transition-colors text-sm backdrop-blur-md border border-white/10 font-medium"
                >
                    חזרה לעריכה
                </button>
            )}
             {isPreview && onSave && (
                <button 
                    onClick={onSave}
                    disabled={isSaving}
                    className="bg-brand-accent text-white py-2 px-6 rounded-full hover:bg-brand-accentHover transition-colors disabled:opacity-70 shadow-lg font-bold border border-transparent"
                >
      ...
      <main className="container mx-auto px-4 py-12 md:py-20 max-w-7xl -mt-10 relative z-10">
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            
            {/* Main Content */}
            <div className="lg:col-span-7 space-y-10">
                <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-100">
                    <h2 className="text-3xl font-bold text-slate-800 mb-8 relative inline-block">
                        אודות הנכס
                        <span className="absolute bottom-[-8px] right-0 w-1/2 h-1 bg-brand-accent rounded-full"></span>
                    </h2>
                    
                    <div className="space-y-6 text-lg text-slate-600 leading-relaxed">
                        <p>{details.enhancedDescription.area}</p>
                        <p>{details.enhancedDescription.property}</p>
                        <p className="font-bold text-slate-800 text-xl mt-4">{details.enhancedDescription.cta}</p>
                    </div>
                </div>
            </div>
            ...
            <aside className="lg:col-span-5 space-y-8">
                ...
                <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl text-center relative overflow-hidden">
                    ...
                    <p className="text-slate-400 mb-2 text-sm uppercase tracking-wider">מחיר מבוקש</p>
                    <div className="flex items-start justify-center gap-1 text-white">
                        ...
                        <span className="text-2xl font-light mt-2 text-brand-accent">{shekelSign}</span>
                    </div>
                </div>
                ...
                <div className="bg-white border border-slate-200 p-8 rounded-3xl shadow-sm text-center">
                    ...
                    <p className="text-slate-500 mb-1">לפרטים נוספים</p>
                    <h3 className="text-2xl font-bold text-slate-800">{details.agentName}</h3>
                    <a 
                        href={`https://wa.me/${details.agentWhatsApp}`} 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="mt-4 inline-flex items-center gap-2 text-brand-accent font-bold hover:text-brand-accentHover transition-colors"
                    >
                        <span>{formatPhoneNumber(details.agentWhatsApp)}</span>
                    </a>
                </div>
            </aside>
        </section>

        <section ref={leadFormRef} className="mt-24 max-w-3xl mx-auto">
      ...
      <footer className="bg-slate-900 text-slate-400 text-center py-8 mt-12 border-t border-slate-800">
        <div className="container mx-auto">
            <p className="text-sm">© כל הזכויות שמורות - נוצר באמצעות מחולל דפי נחיתה לנדל"ן</p>
        </div>
      </footer>
    </div>
  );
...
const UserIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>;
