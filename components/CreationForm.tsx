...
export const CreationForm: React.FC<{ onSubmit: (details: PropertyFormData) => void; isLoading: boolean; defaultAgentName?: string; defaultAgentEmail?: string; }> = ({ onSubmit, isLoading, defaultAgentName = '', defaultAgentEmail = '' }) => {
  const [formData, setFormData] = useState<Omit<PropertyFormData, 'images' | 'logo'>>({
    address: '', 
    description: '', // This acts as the "Property Title" for marketing
    price: '', 
    ...
    <div className="min-h-screen bg-slate-900 py-16 px-4 flex items-center justify-center" dir="rtl">
      <div className="w-full max-w-2xl animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-slate-800/30 p-8 rounded-[2.5rem] border border-white/5 backdrop-blur-xl shadow-2xl">
          <div>
            <label className={labelClasses}>כותרת הנכס (לדוגמה: דירת 4 חדרים משופצת)</label>
            <input name="description" placeholder="מה הופך את הנכס למיוחד?" className={inputClasses} onChange={handleChange} value={formData.description} required />
          </div>

          <div>
            <label className={labelClasses}>כתובת מלאה</label>
            <input name="address" placeholder="רחוב, מספר, עיר" className={inputClasses} onChange={handleChange} value={formData.address} required />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                  <label className={labelClasses}>מחיר (אופציונלי)</label>
          ...
          <div>
            ...
            <div className="relative border-2 border-dashed border-slate-700 rounded-2xl p-4 text-center hover:border-brand-accent transition-colors">
                <input type="file" multiple accept="image/*" onChange={handleFileChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                <p className="text-slate-400 text-sm">לחץ או גרור תמונות כאן ({images.length} נבחרו)</p>
            </div>
          </div>
