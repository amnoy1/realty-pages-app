...
const EmailIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>;
const WhatsAppIcon = () => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="h-6 w-6 text-slate-400" fill="currentColor"><path d="M12 2a10 10 0 0 0-10 10c0 1.88.52 3.64 1.45 5.15L2 22l5.26-1.38A9.95 9.95 0 0 0 12 22a10 10 0 1 0 0-20zm0 18.27c-1.48 0-2.92-.4-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.25 8.25 0 0 1-1.26-4.38c0-4.54 3.68-8.22 8.22-8.22s8.22 3.68 8.22 8.22-3.68 8.22-8.22 8.22zm4.52-6.14c-.25-.12-1.47-.72-1.7-.82s-.39-.12-.56.12c-.17.25-.64.82-.79.98s-.29.17-.54.06c-.25-.12-1.06-.39-2.02-1.24s-1.45-1.95-1.61-2.29c-.17-.34-.02-.52.11-.64s.25-.29.37-.43c.12-.14.17-.25.25-.41s.12-.3-.06-.54c-.18-.25-.56-1.35-.77-1.84s-.41-.41-.56-.41h-.48c-.17 0-.43.06-.64.3s-.82.79-.82 1.95c0 1.15.84 2.27.96 2.43s1.64 2.51 4 3.5c.59.25 1.05.4 1.41.51.62.2 1.17.17 1.62.1.48-.06 1.47-.6 1.68-1.18.21-.58.21-1.08.15-1.18s-.21-.17-.46-.29z"></path></svg>;
...
const LogoIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-10 w-10 text-slate-400"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
);

const DeleteIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
);
      ...
      <div className="container mx-auto max-w-6xl w-full">
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-2xl shadow-2xl p-6 sm:p-10">
          <div className="text-center mb-10">
              ...
              <p className="text-slate-200 text-lg">הזן את פרטי הנכס וצור דף נחיתה מקצועי וממיר ברגע</p>
          </div>
              ...
              <div className="lg:col-span-3 space-y-10">
                <section>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <label htmlFor="description" className="block text-sm font-medium text-slate-100 mb-2">תיאור הנכס</label>
                      ...
                      <p className="text-xs text-slate-300 mt-2">התיאור הזה ישמש ליצירת כותרת ותיאור משופרים באמצעות בינה מלאכותית.</p>
                    </div>
              ...
              <div className="lg:col-span-2">
                <section>
                  ...
                  <div className="space-y-8">
                      <div>
                          <label className="block text-sm font-medium text-slate-100 mb-2">תמונות הנכס (עד 10)</label>
                          <div className="mt-1 flex justify-center p-6 border-2 border-slate-600 border-dashed rounded-lg bg-slate-800 hover:border-slate-500 transition">
                              <div className="space-y-1 text-center">
                                  ...
                                  <p className="text-xs text-slate-300">התמונה הראשונה שתעלה תהיה תמונת הנושא</p>
                              </div>
                          </div>
                      </div>
                      ...
                      {images.length > 0 && (
                          <div>
                              <h3 className="text-sm font-medium text-slate-100 mb-2">גרור כדי לסדר מחדש:</h3>
                      ...
                      <div>
                          <label className="block text-sm font-medium text-slate-100 mb-2">לוגו המשרד (אופציונלי)</label>
                          <div className="mt-1 flex items-center gap-4">
                              ...
                              <label htmlFor="logo-upload" className="cursor-pointer bg-slate-700 py-2 px-4 border border-slate-600 rounded-md shadow-sm text-sm font-medium text-slate-100 hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-blue-500 transition">
...
const InputField: React.FC<{ icon: React.ReactNode; name: string; label: string; placeholder: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; required?: boolean; type?: string; }> = ({ icon, name, label, ...props }) => (
    <div>
        <label htmlFor={name} className="block text-sm font-medium text-slate-100 mb-2">{label}</label>
        <div className="relative">
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3.5">{icon}</div>
            <input id={name} name={name} {...props} className="form-input w-full pr-12 bg-slate-700 border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-blue-500 focus:border-blue-500 transition" />
        </div>
    </div>
);
